...
export function EventsList() {
  ...
  useEffect(() => {
    fetchEvents();
    // No need to start cooldown on initial load
  }, []);
  ...
  const getFormatIcon = (format: string) => {
    switch (format.toLowerCase()) {
      case 'json': return <FileJson className="h-4 w-4 mr-1" />;
      case 'csv': return <FileText className="h-4 w-4 mr-1" />;
      case 'sql': return <Table className="h-4 w-4 mr-1" />;
    ...
    <>
      ...
      {loading && events.length === 0 ? (
        ...
        <div className="grid gap-4">
          {events.map((event) => (
            <Card key={event.id} className={event.success ? '' : 'border-destructive/30'}>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-2 text-sm">
                  ...
                  {event.schemaId && (
                    <div>
                      ...
                      <span className="font-mono text-xs">{event.schemaId.substring(0, 10)}...</span>
                    </div>
                  )}
                </div>
                {event.errorMessage && (
